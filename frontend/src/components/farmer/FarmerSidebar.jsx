...
import { LayoutDashboard, History, FileText, User, X, Sprout, ChevronRight, LogOut } from 'lucide-react';
import api from '../../utils/api';

export default function FarmerSidebar({ isOpen, onClose }) {
    const navigate = useNavigate();

    // Logic kept for mobile logout fallback if needed, though primarily handled in TopBar now
    ...
    const navItems = [
        ...
        { to: '/farmer/invoices', icon: FileText, label: 'Invoices' },
            ...
            >
                ...
                <div className="relative z-10 p-8 pb-4 flex-shrink-0">
                    ...
                    <div className="px-4 py-3 bg-slate-800/50 rounded-xl border border-slate-700/50 mb-2">
                        <p className="text-xs font-bold text-emerald-400 uppercase tracking-wider mb-0.5">Farmer Portal</p>
                        <p className="text-xs text-slate-400">Partner Access</p>
                    </div>
                </div>
                ...
                <nav className="relative z-10 flex-1 px-4 space-y-1.5 overflow-y-auto custom-scrollbar pb-6">
                    ...
                    {/* Mobile Logout (visible only on small screens inside menu) */}
                    <button
                        onClick={handleLogout}
                        className="lg:hidden flex items-center gap-3 px-4 py-3.5 w-full text-red-400 hover:bg-white/5 rounded-xl transition-all duration-300 mt-4"
                    >
